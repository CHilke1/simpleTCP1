...
namespace SimpleTCP
{
    ...
    public class TCP4Header : ProtocolHeader
    {
        ...
        private ushort source_port; // 16 bits
        private ushort destination_port; // 16 bits
        private uint sequence_number; // 32 bits
        private uint acknowledgment_number; // 32 bits
        private ushort data_offset_reserved_control_bits;
        ...
        private ushort window; // 16 bits
        private ushort checksum; // 16 bits
        private ushort urgent_pointer; // 16 bits
        private uint options; // variable
        ...
        public object ipv4Header { get; private set; }
        ...
        public TCP4Header Create(byte[] ipv4Packet, ref int bytesCopied)
        {
            TCP4Header TCP4Header = new TCP4Header();
            ...
            TCP4Header.ipVersion = (byte)((ipv4Packet[0] >> 4) & 0xF);
            TCP4Header.ipLength = (byte)(ipv4Packet[0] & 0xF);
            TCP4Header.ipTypeOfService = ipv4Packet[1];
            TCP4Header.ipTotalLength = BitConverter.ToUInt16(ipv4Packet, 2);
            TCP4Header.ipId = BitConverter.ToUInt16(ipv4Packet, 4);
            TCP4Header.ipOffset = BitConverter.ToUInt16(ipv4Packet, 6);
            TCP4Header.ipTtl = ipv4Packet[8];
            TCP4Header.ipProtocol = ipv4Packet[9];
            TCP4Header.ipChecksum = BitConverter.ToUInt16(ipv4Packet, 10);
            TCP4Header.ipSourceAddress = new IPAddress(BitConverter.ToUInt32(ipv4Packet, 12));
            TCP4Header.ipDestinationAddress = new IPAddress(BitConverter.ToUInt32(ipv4Packet, 16));
            bytesCopied = TCP4Header.Length;
            return TCP4Header;
        }
        /// <summary>
